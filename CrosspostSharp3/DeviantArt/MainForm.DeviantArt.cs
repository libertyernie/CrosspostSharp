...
namespace CrosspostSharp3 {
	public partial class MainForm {
		private async void deviantArtToolStripMenuItem_Click(object sender, EventArgs e) {
			...
			using (var acctSelForm = new AccountSelectionForm<Settings.DeviantArtSettings>(
				...
				async () => {
					using (var f = new DeviantArtAuthorizationCodeForm(
						...
						if (f.ShowDialog(this) == DialogResult.OK) {
							...
							return new[] {
								new Settings.DeviantArtSettings {
					...
					return Enumerable.Empty<Settings.DeviantArtSettings>();
				}
			)) {
